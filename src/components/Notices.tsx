...
const TEXT = {
  sv: {
    data: 'All statistik kommer från SCB:s öppna statistikdatabas och är licensierad CC0 1.0 Universal. SCB kräver ingen källhänvisning. SCB anger också att du inte ska ange SCB som källa för siffror du själv har bearbetat — därför märks varje härledd siffra i den här databasen som projektets egen beräkning, med SCB-tabellen den bygger på angiven. Den här webbplatsen är inte ett samarbete med SCB och framställs inte som ett sådant.',
    boundaries:
  ...
  en: {
    data: "All statistics come from Statistics Sweden's open statistical database and are licensed CC0 1.0 Universal. Statistics Sweden requires no attribution. They also ask that you do not cite them as the source for figures you have processed yourself — so every derived figure here is labelled as this project's own calculation, naming the table it was built from. This site is not a cooperation with Statistics Sweden and is not presented as one.",
    boundaries:
