...
/**
 * Arrow-key movement between municipalities, shared by the site and the kitchen.
 *
 * Pure arithmetic over a neighbour graph and a set of centroids: no DOM, no Node, so both programs
 * may import it. The site navigates by it; the kitchen proves every layout it publishes reachable
 * by it (`strandedIn`).
 */
...
/**
 * The municipalities no arrow key from anywhere can land on — empty when every one is reachable.
 *
 * Plan 3 proved this for the geographic map and Plan 4 for the one bubble layout, both by hand and
 * both pinned by a test against the published file. Plan 21 publishes a layout per indicator, so
 * the proof has to be made by the kitchen for each one before it is written, and re-made by the
 * site's tests against what was written. This is the function both use, so a layout the kitchen
 * called reachable is reachable by exactly the rule the site navigates with.
 */
export function strandedIn(ctx: NavContext): string[] {
  const reached = new Set<string>()
  for (const from of ctx.centroids.keys()) {
    for (const direction of DIRECTIONS) {
      const to = step(from, direction, ctx)
      if (to) reached.add(to)
    }
  }
  // Plain code-unit order, never localeCompare: the kitchen writes this into an error message and
  // a test compares it, and neither may depend on the running machine's collation.
  return [...ctx.centroids.keys()].filter((code) => !reached.has(code)).sort()
}
