...
Last reviewed: 2026-09-21, when issue #32 turned out to be a focus defect rather than a test flake.
...
| Keyboard behaviour in Chromium, Firefox and WebKit                | `e2e/keyboard.spec.ts`                                           | tab order, arrow navigation, the focus ring, the slider, the skip link, and the focus a deep link does NOT move |
...
| **A deep link moved focus below the map.** The profile focused its own heading whenever it appeared, arrival included, so a keyboard visitor opening `/en/malmo-1280/` was dropped past the skip link, the search, the year and the map with no way forward — at an unpredictable moment after the page was readable, because the panel cannot render until the pantry has been fetched | the browser suite, as three WebKit-only failures ([#32](https://github.com/aniisabihi/atlas290/issues/32)) | a deep link moves no focus; opening, closing and going back still declare theirs. [ADR-0018](decisions/0018-the-focus-a-link-never-asked-for.md) |
...
| **The theme button contradicted itself.** It carried `aria-pressed` beside a label naming the action, so in the dark theme a screen reader announced "Switch to the light theme, toggle button, pressed"                                                                                                                                                                                | reading the markup, in the editorial pass (ADR-0025) — no tool flags a state that disagrees with a name    | the state is gone; the name is the action, pinned by `ThemeToggle.test.tsx`                                                                      |
| **Accents on the plate at about 2.2:1 in the dark theme.** The table's selected-row mark and header hover used the theme accent, a pale blue, on the always-white plate                                                                                                                                                                                                                 | looking, in the editorial pass                                                                             | a fixed `--on-plate-accent`, asserted in `tokens.test.ts`                                                                                        |
...
- **A link changes what is on screen, never where the visitor is.** Opening a profile moves focus
  ...
  move focus without failing. The cost, stated rather than buried: Escape closes the profile from
  inside it or from the map, not from the top of a page that has just loaded.
  [ADR-0018](decisions/0018-the-focus-a-link-never-asked-for.md).
