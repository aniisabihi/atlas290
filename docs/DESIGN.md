...
| **Phones are first-class, and see the cartogram by default**                                                               | The kitchen already computes the bubble layout. Bubbles make every municipality a visible, tappable dot and waste no width on a country three times taller than wide. Only the animated morph is deferred. Since [decision 0023](decisions/0023-bubbles-sized-by-the-measure.md) there is one layout per indicator, sized by the measure. |
...
**The map is 290 SVG paths.** Small enough that the browser treats it as ordinary content, which is where the accessibility comes from. The cartogram layouts are precomputed in the kitchen, one per indicator since [decision 0023](decisions/0023-bubbles-sized-by-the-measure.md), each proven reachable with the arrow keys before it is published; on desktop the browser slides shapes between the map and the drawn indicator's layout, on phones the bubbles are simply the default view with the panel as a bottom sheet.
