...
> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking. **Do not start until the architect has said OK to this file.**
...
- [ ] Give `SearchBox` its own root class (`.search`) so the wrapper styling travels with it, and
      let the bar and the profile header pass a `variant` for width only.
- [ ] Move the compare search into `.profile-header`, after the name, before "Stäng".
- [ ] Test (`SearchBox.test.tsx`, `ComparePanel.test.tsx`): both placements render the same
      roles, labels and listbox; the compare field's accessible name is "Jämför med…" in Swedish
      and its English twin.
- [ ] Verify: `yarn vitest run src/components/SearchBox.test.tsx src/components/ComparePanel.test.tsx`.

### Task 2 — The profile moves into the reading column

- [ ] `App.tsx`: render `ProfilePanel` (with `ComparePanel` under it) inside `.reading-column`
      after `AboutIndicator` when `!narrow`; keep the sheet path unchanged when `narrow`.
- [ ] `FactsStrip` stays after `.layout`.
- [ ] Focus on open still lands on `#profile-heading`; the reading column is not sticky.
- [ ] Test (`App.test.tsx`): with a selection, the profile heading is a descendant of the reading
      column at desktop width and of the sheet at phone width; DOM order is map → reading column
      → facts.
- [ ] e2e (`state.spec.ts`): after clicking a shape at 1440×900 the profile heading is in the
      viewport without scrolling the map plate out of view (`boundingBox` of `#map` unchanged).

### Task 3 — Disclosures

- [ ] `app.css`: a shared `summary` rule with an inline SVG chevron (`::before`), rotation on
...
- [ ] Test (`AboutIndicator.test.tsx`): toggling reveals the description; the summary is a
      `button`-like disclosure with `aria-expanded` state readable via the native element.
- [ ] axe in both themes still green (Task 8).

### Task 4 — The table takes the plate's height

- [ ] `.view-column` owns the stage height: one `--stage-height` derived from the plate (the
      `map-frame` today) and applied to `.table-scroll` as `max-height` with `overflow: auto`.
- [ ] `thead th { position: sticky; top: 0 }` with a background token so rows never show
      through; the caption stays visible.
- [ ] Phone: same rule against the bubble stage's height.
- [ ] Test (`DataTable.test.tsx`): 290 rows are all in the DOM; the scroll container has the
      class the CSS binds to.
- [ ] e2e (`reflow.spec.ts`): at 320 px no horizontal document scroll; (`state.spec.ts`) with
      `?t=1` the document height is within 1.5 × viewport height at 1440×900.

### Task 5 — The tooltip

- [ ] `MapTooltip.tsx`: renders name, value and rank for a code, positioned from a pointer
      position or a shape's `getBBox()`; `aria-hidden="true"`; hidden while `useMorph` reports
      progress strictly between 0 and 1.
- [ ] `MapCanvas.tsx`: `onPointerEnter`/`onPointerMove`/`onPointerLeave` on the `<svg>` with event
...
- [ ] A `:hover`/`[data-hover]` stroke on the shape, two-tone like the focus ring, so the pointer
      target is visible against every class fill.
- [ ] Test (`MapCanvas.test.tsx`): pointer move over a shape renders the tooltip text equal to
      the shape's `aria-label` reading; leaving hides it; focus shows it; the live region text is
      unchanged by hover.
- [ ] e2e (`keyboard.spec.ts`): arrow-key focus moves the tooltip with the shape.
- [ ] Verify the frame budget claim of decision 0004 is untouched: no per-path handler, no state
      update during morph.

### Task 6 — Hover connects the page

- [ ] `SimilarPlaces` chips and facts links expose `data-code` where their href names a
      municipality; `App.tsx` holds a `highlight: string | null` and passes it to `MapCanvas`,
      which sets `data-highlight` on that shape.
- [ ] The selected municipality's shape is ringed (`data-selected`) while the profile is open.
- [ ] `Legend` accepts a `hoveredValue` and draws a tick under the class it falls in; no text
      changes.
- [ ] Tests (`SimilarPlaces.test.tsx`, `FactsStrip.test.tsx`, `Legend.test.tsx`): hover sets and
...
- [ ] `.profile-row` gets a narrow container-query layout: name on its own line, value and rank
      beneath, trend full width; no `white-space: nowrap` on the absence text.
- [ ] Test (`ProfilePanel.test.tsx`): each row renders name, value, rank and trend in the
      expected order regardless of width (structure only; jsdom measures nothing).
- [ ] Screenshot at 390 px checked by eye for the two rows that overprinted.

### Task 8 — Prove it, then write it down

- [ ] `yarn typecheck && yarn lint && yarn test && yarn build`; `yarn e2e` in all three engines.
- [ ] axe in both themes on every covered state plus a hovered-shape state and the table view.
- [ ] `yarn budget`: performance ≥ 80 (expected unchanged; the tooltip adds under 2 kB).
- [ ] `yarn kitchen publish` leaves `public/pantry/` byte-identical.
- [ ] Screenshots: front page, profile, compare, table, phone profile, both themes.
- [ ] `docs/decisions/0010-the-place-beside-the-map.md` with D1–D7 and the measured numbers;
...
- [ ] Final review, then open a PR against `main` from `feat/plan-11-the-place-beside-the-map`.
